...
const h = (n) => {
    return (socket) => {
        ...
        socket.on('data', data => {
            console.log(`server ${n} receive from ${socket.remoteAddress + ':' + socket.remotePort}: ${data.readInt32LE()}`);
...
net.createServer(h(PORT1)).listen(PORT1).on('listening', () => console.log('TCP-server: 127.0.0.1:',PORT1))
net.createServer(h(PORT2)).listen(PORT2).on('listening', () => console.log('TCP-server: 127.0.0.1:',PORT2))
